...
describe('Method Not Allowed with intentional 404 from a registered handler', () => {
  const app = new Hono()
  app.use(methodNotAllowed({ app }))
  app.get('/users/:id', (c) => c.notFound())
  app.post('/users/:id', (c) => c.text('Created'))

  it('Should preserve 404 when the request method is registered but the handler returns 404', async () => {
    const res = await app.request('/users/999')
    expect(res.status).toBe(404)
  })

  it('Should still return 405 when the request method is not registered', async () => {
