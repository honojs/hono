...
describe("compose", () => {
  ...
  it("Request", async () => {
    ...
    const composed = compose<C>(middleware, onError, onNotFound);
...
describe("compose with returning a promise, non-async function", () => {
  ...
  it("Response", async () => {
    const c: C = { req: {}, res: {}, finalized: false };
    const composed = compose<C>(handlers, onError, onNotFound);
    const context = await composed(c);
    expect(context.res["message"]).toBe("new response");
  });
});
...
describe("compose with Context - 404 not found", () => {
  ...
  it("Should return 404 Response - async handler", async () => {
    const c: Context = new Context(req);
    const composed = compose<Context>(middleware, onError, onNotFoundAsync);
    const context = await composed(c);
    expect(context.res).not.toBeNull();
    expect(context.res.status).toBe(404);
...
describe("compose with Context - 500 error", () => {
  ...
  it("Run all the middlewares", async () => {
    ...
    const composed = compose(middlewares, onError, onNotFound);
    await composed(ctx);
...
describe("Compose", function () {
  it("should get executed order one by one", async () => {
    ...
    await compose(stack, onError, onNotFound)({ res: null, finalized: false });
  ...
  it("should be able to be called twice", async () => {
    ...
    const fn = compose(stack, onError, onNotFound);
  ...
  it("should work with 0 middleware", async () => {
    await compose([], onError, onNotFound)({ res: null, finalized: false });
  });
  ...
  it("should reject on errors in middleware", async () => {
    ...
    try {
      await compose(stack, onError, onNotFound)({
        res: null,
        finalized: false,
      });
  ...
  it("should keep the context", async () => {
    ...
    await compose(stack, onError, onNotFound)(ctx);
  });
  ...
  it("should compose w/ next", async () => {
    ...
    await compose([], onError, onNotFound)(
  ...
  it("should return last return value", async () => {
    ...
    const res = await compose<C>(stack, onError, onNotFound)({
      val: 0,
      res: null,
      finalized: false,
    });
  ...
  it("should not get stuck on the passed in next", async () => {
    ...
    await compose<C>(middleware, onError, onNotFound)(
      ctx,
      (ctx: C, next: Function) => {
        ctx.next++;
        return next();
