...
export class AdapterRequest implements Request {
  ...
  constructor(
    input: string | URL | AdapterRequest,
    init?: RequestInit
  ) {
    this.cache = input instanceof AdapterRequest ? input.cache : (init?.cache ?? 'default')
    this.credentials = input instanceof AdapterRequest ? input.credentials : (init?.credentials ?? 'same-origin')
    ...
    this.method = normalizeMethod(input instanceof AdapterRequest ? input.method : (init?.method ?? 'GET'))
    this.mode = input instanceof AdapterRequest ? input.mode : (init?.mode ?? 'cors')
    this.url = input instanceof AdapterRequest ? input.url : (typeof input === 'string' ? input : input.toString());
    ...
    this.referrerPolicy = input instanceof AdapterRequest ? input.referrerPolicy : (init?.referrerPolicy ?? '');
    ...
    this.body = input instanceof AdapterRequest ? input.body : bodyInitToBody(init?.body as BodyInit ?? null);
  ...
  clone(): AdapterRequest {
    ...
    this.body = oldBody
    return newBody ? new AdapterRequest(this, {
      body: newBody,
...
export class AdapterResponse implements Response {
  ...
  clone(): AdapterResponse {
    const [oldBody, newBody] = this.body ? this.body.tee() : [null, null];
    this.body = oldBody;
    return newBody ? new AdapterResponse(newBody, {
